...
public class CollectionInjector {
	...
	public CollectionInjector(CollectionFactory collectionFactory, SelectionStrategy strategy,
	...
	public void inject(InjectionDetails injectionDetails) {
		for (Object injectCollections : injectionDetails.getInjectCollections()) {
			inject(injectCollections, injectionDetails.getMocks(), injectionDetails.getSpies(), injectionDetails.getInjectableCollectionSet(),
					injectCollections.getClass());
		}
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void inject(Object injectCollections, OrderedSet<Object> mocks, OrderedSet<Object> spies,
			...
			if (type instanceof ParameterizedType) {
				...
				if (Collection.class.isAssignableFrom(rawType)) {
					...
					if (collectionType != null) {
						Collection collection = getCollection(mocks, spies, collectionOfMocksFieldSet, rawType, collectionType);
						if (collection != null) {
				...
				if (clazz.isArray()) {
					Set strategyInjectables = strategy.select(mocks, spies, clazz.getComponentType());
		...
		if (superclass != Object.class && superclass != null) {
			inject(injectCollections, mocks, spies, collectionOfMocksFieldSet, superclass);
		}
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	private Collection getCollection(OrderedSet<Object> mocks, OrderedSet<Object> spies, CollectionOfMocksFieldSet collectionOfMocksFieldSet,
		...
		if (collectionOfMocksField != null) {
			...
			strategyInjectables = strategy.select(mocks, spies, (Class) collectionType);
		}
		if (!strategyInjectables.isEmpty()) {
