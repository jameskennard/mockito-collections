...
public class CollectionInjector {
	...
	public CollectionInjector(CollectionFactory collectionFactory,
			InjectableSelectionStrategy strategy) {
		...
		for (Field field : fields) {
			...
			if (genericType instanceof ParameterizedType) {
				...
				if (Collection.class.isAssignableFrom(rawType)) {
					...
					Set injectables = strategy.getInjectables(injectionDetails.getInjectables(), (Class)typeArguments[0]);
